...
#ifndef S2_S2CLOSESTPOINTQUERY_BASE_H_
#define S2_S2CLOSESTPOINTQUERY_BASE_H_
...
S2ClosestPointQueryBase<Distance, Data>::FindClosestPoint(
    Target* target, const Options& options) {
  //DCHECK_EQ(options.max_points(), 1);
...
void S2ClosestPointQueryBase<Distance, Data>::FindClosestPoints(
  ...
  if (options.max_points() == 1) {
    ...
    //DCHECK(std::is_sorted(results->begin(), results->end()));
  }
}
...
#endif  // S2_S2CLOSESTPOINTQUERY_BASE_H_
